...
Deno.serve(async (req: Request) => {
    ...
    try {
        const r = await supabase.from("qb_sync_history").insert({ sync_type: "purchase_orders", direction: "qb_to_supabase" }).select().single();
        syncRecord = r.data;
    ...
    try {
        ...
        (suppliers || []).forEach((s: any) => supplierByName.set((s.name || "").trim().toLowerCase(), s.id));

        const { data: products } = await supabase.from("products").select("id, sku, name, qb_item_id");
        const productByQbId = new Map<string, any>();
        (products || []).forEach((p: any) => { if (p.qb_item_id) productByQbId.set(String(p.qb_item_id), p); });
        ...
        while (hasMore) {
            ...
            if (purchaseOrders.length === 0) { hasMore = false; break; }
            ...
            for (const po of purchaseOrders) {
                ...
                try {
                    ...
                    const orderDate = po.TxnDate ? new Date(po.TxnDate).toISOString().slice(0, 10) : null;
                    const dueDate = po.DueDate ? new Date(po.DueDate).toISOString().slice(0, 10) : null;
                    const totalAmt = po.TotalAmt ?? 0;
                    const status = mapQbPoStatus(po);
                    const notes = [po.PrivateNote, supplierId ? null : `QB Vendor: ${vendorName}`].filter(Boolean).join("\n") || null;

                    let existing: { id: string } | null = null;
                    const byPo = await supabase.from("purchase_orders").select("id").eq("po_number", poNumber).maybeSingle();
                    existing = byPo.data;
                    if (!existing && byPo.error && byPo.error.code !== "PGRST116") throw new Error(byPo.error.message);
                    ...
                    if (existing && existing.id) {
                        await supabase.from("purchase_orders").update(poData).eq("id", existing.id);
                        poRecordId = existing.id;
                        await supabase.from("purchase_order_items").delete().eq("purchase_order_id", existing.id);
                        stats.updated++;
                    } else {
                        const { data: inserted, error: insertErr } = await supabase
                            .from("purchase_orders").insert(poData).select("id").single();
                        if (insertErr) throw insertErr;
                        poRecordId = inserted.id;
                        stats.created++;
                    }

                    const lines = (po.Line || []).filter((l: any) =>
                        l.DetailType === "ItemBasedExpenseLineDetail" || l.DetailType === "PurchaseOrderItemLineDetail" || (l.ItemBasedExpenseLineDetail && l.Amount != null)
                    );
                    const items: any[] = [];
                    for (const line of lines) {
                        const detail = line.ItemBasedExpenseLineDetail || line.PurchaseOrderItemLineDetail || line;
                        ...
                        const itemName = (itemRef?.name || line.Description || "Item").trim();
                    ...
                    if (items.length > 0) {
                        ...
                        if (itemsErr) stats.errors.push({ qb_po: poNumber, error: `Items: ${itemsErr.message}` });
                    }
                } catch (err: any) {
                    stats.failed++;
                    stats.errors.push({ qb_po: po.DocNumber || po.Id, error: err.message });
                }
            }
        ...
        return new Response(JSON.stringify({ success: true, sync_type: "purchase_orders", stats }), {
