...
Deno.serve(async (req: Request) => {
    ...
    try {
        if (event.type === "checkout.session.completed") {
            await handleCheckoutComplete(event.data.object as unknown as StripeSession, supabase);
        } else if (event.type === "payment_intent.payment_failed") {
            await handlePaymentFailed(event.data.object as unknown as StripePaymentIntent, supabase);
        }
...
async function handleCheckoutComplete(session: StripeSession, supabase: ReturnType<typeof createClient>) {
    ...
    console.log(`[stripe-webhook] Saving order ${orderId} for ${orderData.customer_email}`);
...
async function handlePaymentFailed(pi: StripePaymentIntent, supabase: ReturnType<typeof createClient>) {
...
async function triggerQBSync(_supabase: ReturnType<typeof createClient>, orderId: string) {
