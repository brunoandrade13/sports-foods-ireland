...
function mapQbStatusToOrderStatus(invoice: any): { status: string; payment_status: string; financial_status: string } {
    const balance = invoice.Balance || 0;
    const total = invoice.TotalAmt || 0;

    if (balance === 0 && total > 0) {
        return { status: "delivered", payment_status: "paid", financial_status: "paid" };
    } else if (balance < total && balance > 0) {
        return { status: "confirmed", payment_status: "paid", financial_status: "partially_paid" };
    } else {
        return { status: "pending", payment_status: "pending", financial_status: "pending" };
    }
}
...
Deno.serve(async (req: Request) => {
    ...
    const { data: syncRecord } = await supabase.from("qb_sync_history").insert({ sync_type: "orders", direction: "qb_to_supabase" }).select().single();
    const stats = { processed: 0, created: 0, updated: 0, skipped: 0, failed: 0, errors: [] as any[] };

    try {
        const { accessToken, realmId } = await getValidToken(supabase);

        // Build lookup maps
        const { data: customers } = await supabase.from("customers").select("id, qb_customer_id").not("qb_customer_id", "is", null);
        const customerMap = new Map();
        (customers || []).forEach((c: any) => customerMap.set(c.qb_customer_id, c.id));

        const { data: products } = await supabase.from("products").select("id, sku, name, qb_item_id");
            ...
            for (const invoice of invoices) {
                ...
                try {
                    ...
                    const orderData: Record<string, any> = {
                        order_number: invoice.DocNumber ? `QB-${invoice.DocNumber}` : `QB-${invoice.Id}`,
                        ...
                        created_at: invoice.MetaData?.CreateTime || invoice.TxnDate || new Date().toISOString(),
                    };
                    ...
                    const lineItems = (invoice.Line || []).filter((l: any) => l.DetailType === "SalesItemLineDetail");
                    const orderItems = [];

                    for (const line of lineItems) {
                        const detail = line.SalesItemLineDetail;
                        const qbItemId = detail?.ItemRef?.value ? String(detail.ItemRef.value) : null;
                        ...
                        orderItems.push({
                            ...
                            product_name: matchedProduct?.name || line.Description || qbNameRaw || "Unknown",
                            product_sku: matchedProduct?.sku || qbNameRaw || qbItemId || "",
                            quantity: detail?.Qty || 1,
                            unit_price: detail?.UnitPrice || line.Amount || 0,
                            total_price: line.Amount || 0,
                            total: line.Amount || 0,
                            created_at: invoice.MetaData?.CreateTime || new Date().toISOString(),
                        });
                    }

                    if (orderItems.length > 0) {
                        const { error: itemsError } = await supabase.from("order_items").insert(orderItems);
                        if (itemsError) stats.errors.push({ qb_invoice: invoice.DocNumber, error: `Items error: ${itemsError.message}` });
                    }

                    await supabase.from("orders").update({
                        item_count: orderItems.reduce((s: number, i: any) => s + i.quantity, 0),
                    }).eq("id", orderId);
                    ...
                    stats.failed++;
                    stats.errors.push({ qb_invoice: invoice.DocNumber || invoice.Id, error: err.message });
                }
            }
        ...
        await supabase.from("qb_sync_history").update({
            ...
            records_updated: stats.updated, records_failed: stats.failed,
            errors: stats.errors, completed_at: new Date().toISOString(), status: "completed",
        }).eq("id", syncRecord?.id);

        return new Response(JSON.stringify({ success: true, sync_type: "orders", stats }), {
