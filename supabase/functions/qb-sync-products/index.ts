...
async function getValidToken(supabase: any) {
    const { data: integration, error } = await supabase
        .from("qb_integration").select("*").limit(1).single();
    ...
    if (expiresAt.getTime() - Date.now() < 5 * 60 * 1000) {
        ...
        await supabase.from("qb_integration").update({
            ...
            token_expires_at: new Date(Date.now() + tokens.expires_in * 1000).toISOString(), updated_at: new Date().toISOString(),
...
Deno.serve(async (req: Request) => {
    ...
    const { data: syncRecord } = await supabase.from("qb_sync_history").insert({ sync_type: "products", direction: "qb_to_supabase" }).select().single();
    const stats = { processed: 0, created: 0, updated: 0, skipped: 0, failed: 0, errors: [] as any[], unmatched: [] as any[] };
    ...
    try {
        ...
        const taxCodeMap = new Map<string, { name: string; rate: number; isTaxable: boolean }>(); // TaxCode.Id → { name, rate, isTaxable }

        // Step 1: Fetch all TaxRate entities
        try {
            const trResult = await qbQuery(accessToken, realmId, "SELECT * FROM TaxRate");
            const taxRates = trResult?.QueryResponse?.TaxRate || [];
            for (const tr of taxRates) {
                let rate = Number(tr.RateValue) || 0;
                if (!tr.RateValue && tr.EffectiveTaxRate && tr.EffectiveTaxRate.length > 0) {
                    const now = new Date().toISOString();
                    const activeRate = tr.EffectiveTaxRate.find((r: any) => {
                        const effective = r.EffectiveDate ? new Date(r.EffectiveDate).toISOString() : null;
                    ...
                    if (activeRate) {
                        ...
                        rate = Number(tr.EffectiveTaxRate[tr.EffectiveTaxRate.length - 1].RateValue) || 0;
                    }
                }
        ...
        try {
            const tcResult = await qbQuery(accessToken, realmId, "SELECT * FROM TaxCode");
        ...
        const { data: supaProducts } = await supabase.from("products").select("id, sku, name, qb_item_id, cost_price_eur, vendor, barcode");
            ...
            for (const item of items) {
                ...
                try {
                    ...
                    if (!matchedProduct) {
                        ...
                        if (!matchedProduct && qbName) matchedProduct = productBySku.get(qbName);
                    }
                    ...
                    if (!matchedProduct) {
                        ...
                        if (!matchedProduct && codeInSku) matchedProduct = productBySku.get(codeInSku);
                    }

                    // Priority 4: Match by Description (Fuzzy match against Supabase name)
                    if (!matchedProduct && item.Description) {
                        matchedProduct = productByName.get(normalizeForMatch(item.Description));
                    }
                    ...
                    if (!matchedProduct && item.Description) {
                        ...
                        for (const [supaNorm, product] of productByName.entries()) {
                            if (supaNorm.length > 5 && (qbDescNorm.includes(supaNorm) || supaNorm.includes(qbDescNorm))) {
                    ...
                    if (!matchedProduct && item.PurchaseDesc) {
                        ...
                        for (const [supaNorm, product] of productByName.entries()) {
                            if (supaNorm.length > 5 && (qbPDNorm.includes(supaNorm) || supaNorm.includes(qbPDNorm))) {
                                matchedProduct = product;
                                break;
                            }
                        }
                    ...
                    if (!matchedProduct && item.Name) {
                        ...
                        for (const [supaNorm, product] of productByName.entries()) {
                            if (supaNorm.length > 5 && (qbNorm2.includes(supaNorm) || supaNorm.includes(qbNorm2))) {
                                matchedProduct = product;
                                break;
                            }
                        }
                    ...
                    if (!matchedProduct) {
                        for (const [sku, product] of productBySku.entries()) {
                            if (sku.length > 3 && (qbName.includes(sku) || qbSku.includes(sku) || qbDesc.includes(sku))) {
                                matchedProduct = product;
                                break;
                            }
                        }
                    ...
                    if (item.SalesTaxCodeRef?.value) {
                        ...
                        updateData.qb_sales_tax_code = taxInfo?.name || item.SalesTaxCodeRef.name || null;
                    ...
                    stats.failed++;
                    stats.errors.push({ qb_id: item.Id, name: item.Name, error: err.message });
                }
            }
        ...
        await supabase.from("qb_sync_history").update({
            records_processed: stats.processed, records_created: stats.created,
            records_updated: stats.updated, records_skipped: stats.skipped,
            records_failed: stats.failed, errors: [...stats.errors, ...stats.unmatched.map((u: any) => ({ type: "unmatched", ...u }))],
            completed_at: new Date().toISOString(), status: "completed",
        }).eq("id", syncRecord?.id);

        // Build tax codes summary for the response
        const taxCodesSummary = Array.from(taxCodeMap.entries()).map(([id, info]) => ({
            qb_tax_code_id: id,
            name: info.name,
            vat_rate: info.rate,
        ...
        return new Response(JSON.stringify({
            success: true, sync_type: "products",
