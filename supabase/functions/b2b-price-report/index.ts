...
const WHOLESALE_META_KEYS = [
  ...
  "_wholesale_price",
  "wwp_wholesale_price",
  "_wwp_wholesale_price",
  "wholesale_price_eur",
  "_wholesale_price_eur",
  "b2b_price",
  "_b2b_price",
];
...
function findMatchedProduct(
  ...
  if (matched) return matched;
  if (item.Description)
    matched = productByName.get(normalizeForMatch(item.Description));
  if (matched) return matched;
  ...
  for (const [sku, product] of productBySku.entries()) {
    if (
      sku.length > 3 &&
      (qbName.includes(sku) || qbSku.includes(sku) || qbDesc.includes(sku))
    )
      return product;
  }
  return null;
...
function getWholesaleFromMeta(
  meta_data: { key: string; value?: string }[] | undefined,
): number | null {
...
Deno.serve(async (req: Request) => {
  ...
  const report: {
    supabase: {
      total: number;
      with_wholesale: number;
      without_wholesale: number;
    };
    quickbooks: {
    ...
    woocommerce: {
      configured: boolean;
      products_with_wholesale: number;
      matched: number;
      missing_in_supabase: number;
      rows: {
        woo_id: number;
  ...
  try {
    ...
    report.supabase.with_wholesale = products.filter(
      (p: any) =>
        p.wholesale_price_eur != null && Number(p.wholesale_price_eur) > 0,
    ).length;
    report.supabase.without_wholesale = products.filter(
      (p: any) =>
        p.wholesale_price_eur == null || Number(p.wholesale_price_eur) <= 0,
    ).length;
    ...
    try {
      ...
      while (hasMore) {
        ...
        for (const item of items) {
          ...
          const purchaseCost =
            item.PurchaseCost != null || (item as any).purchaseCost != null
              ? Number((item as any).PurchaseCost ?? (item as any).purchaseCost)
              : undefined;
          ...
          if (report.quickbooks.prices_found.length < MAX_QB_PRICES_DISPLAY) {
            report.quickbooks.prices_found.push({
              ...
              purchase_cost:
                purchaseCost != null && !isNaN(purchaseCost)
                  ? purchaseCost
                  : undefined,
            });
          }
          ...
          const matched = findMatchedProduct(
            item,
            productByQbId,
            productBySku,
            productByName,
          ...
          const hasWholesale =
            matched.wholesale_price_eur != null &&
            Number(matched.wholesale_price_eur) > 0;
          if (!hasWholesale) {
    ...
    if (hasWoo) {
      try {
        ...
        while (true) {
          ...
          if (!res.ok)
            throw new Error(
              `WooCommerce API ${res.status}: ${await res.text()}`,
            );
          ...
          for (const prod of list) {
            ...
            const matched = sku
              ...
              : productByName.get(normalizeForMatch(name));
            if (matched) {
    ...
    return new Response(
      JSON.stringify({ success: false, error: err.message, report }, null, 2),
      {
        status: 500,
        headers: {
          ...corsHeaders,
